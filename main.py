...
class Game:
    def __init__(self):
        ...
        self.letters_text = UIElement(75, 70, "Not Enough Letters", WHITE)
    ...
    def create_tiles(self):
        for row in range(6):
            self.tiles.append([])
            for col in range(5):
                self.tiles[row].append(Tiles((col * (TILESIZE + GAPSIZE)) + MARGIN_X, (row * (TILESIZE + GAPSIZE)) + MARGIN_Y))
    ...
    def row_animation(self):
        #row shaking if not enough letters entered
    ...
    def reveal_animation(self, tile, colour):
        # revel colours animation when user input the whole word
        screen_copy = self.screen.copy()
    ...
    def check_letters(self):
        #algorithm to check if the letters entered correspond to any of the letters in the actual word
                    ...
                    if len(self.text) == 5:
                        ...
                        if self.text == self.word or self.current_row + 1 == 6:
                            ...
                            if self.text != self.word:
                                self.end_screen_text = UIElement(65, 700, f"THE WORD WAS: {self.word}", WHITE)
                            #player wins, send win message
                            else:
                                self.end_screen_text = UIElement(65, 700, "YOU GUESSED CORRECT", WHITE)
